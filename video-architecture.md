...
└─────────────────────────────────────────────────────────────────────────────────────┘
     ...
     └────────────────────────────────────┘    └────────────────────────────────────┘
                 ...
                 └─────────────────┘               └─────────────────┘
                                          ...
                                          │         (CPU 回退方案)              │
                                          │                                    │
                                          │  applyCPUFilters(pixelData, params)│
                                          │  createCPUFilterProcessor(params)  │
                                          │                                    │
                                          │  // 与 WebGL 着色器算法完全一致     │
                                          └────────────────────────────────────┘
```
...
6. **CPU 滤镜回退**（`filters.ts`）
   - 当 WebGL 不可用时提供 CPU 处理方案
   - `applyCPUFilters`：直接应用滤镜到像素数据
   - `createCPUFilterProcessor`：创建预计算参数的处理器，适合多帧处理
   - 算法与 WebGL 着色器完全一致

7. **组件化错误处理**（`ErrorOverlay`）
   - 将加载失败页面抽离为独立组件
